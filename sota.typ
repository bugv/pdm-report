...
We will _not_ consider metastable phases that are on the convex hull but not equilibrium phases (phases which are thermodynamically accesible by a change in composition). 
...
These case are illustrated in @f_hull_explanation.
...
This can lead to the formation of many out of global equiblirium intermediates, which are locally at equilibrium at the interface.
...
In practice, this difference can be observed either in some in-situ experiments where two regimes are observed @kamm2022RelativeKinetics @tan2026OriginReaction or in experiments with varying particle size @kupp2014ParticleSize, where the onset of phase formation is the same regardlees of particle size while reaction completion happens at lower temperatures for smaller particle sizes for which less range transport is required.
However, since both of these regimes are closely related and are related to the same energy scale, they do not necessarily need to be distinguised in practice. 
...
Tamman's rule states that a precursor is "activated" at 2/3 of its melting temperature, after which they may start reacting.  @merkle2005TammannRule
Merkle and Maier propose that this is related to the thermal generation of point defected, which at higher temperatures would interact, leading to breakdown and melting, but which at lower temperatures would activate diffusion, thus relating diffusion and melting.
We note that, while solid state reactions involve multiple precursors, which precursor to chose for the application of Tamman's rule is not clear.
...
Classical nucleation theory can be used to approximate, from reaction free energy $Delta G_r$ and surface energies $gamma$, the energetic barrier $Delta G^*$ assiociated with nucleation, or equivalently the critical nucleus size $r^*$ required for growth via @aykol2018ThermodynamicLimit @zeng2024SelectiveFormation
$
  Delta G^* ~ (gamma ^3)/(Delta G_r ^2)
$
One can then estimate the nucleation rate $Q$ via 
$
  Q ~ A exp(- B (Delta G^*) / (n^2 k_B T))
$
where the prefactor $A$ depends, among other factors, on diffusion rates, relating to the sort and medium interdiffusion discussed in the previous section, $n$ is atomic density, and $B$ is a geometric factor.
...
This is the only report that we have found which definitely shows that nucleation can have an impact on a solid state synthesis reaction synthesis pathway and form metastable polymorphs.
...
== Synthesis of metastable materials and soft chemistry routes
<s_sota_soft>
...
+ Topochemical routes. In this case the crystal structure of a parent phase is chemically modified while keeping a similar lattice structure, bypassing the need for nucleation. This route inclues ion exchange, (de)hydration, (de)intercalation routes where typically small ions (eg. H, Li) are added, removed, or swapped in a crystal structure. @parija2018TraversingEnergy 
  These routes can also be grouped with epitaxial synthesis, where similarly the underlying substratestructure is preserved in atom-by-atom growth.
...
+ Reactions in well mixed or fast diffusion environments, which allows for much lower reaction temperatures. The stategy here is to shift kinetic control from diffusion to nucleation.  @parija2018TraversingEnergy @cordova2020SynthesisMetastable. 
...
To comment on this question, #textcite(<sun2016ThermodynamicScale>) introduce the idea of "remnant metastability," and propose that phases observed are often remnants of coniditons in which they have the lowest free energy.
This includes phases thermodynamically stable at high temperature, high pressures, in different atmospheres, strain and electromagnetic fields, or other extrenal constraints, and then quenched to room conditions, but is also understood to included phases stable at the nanoscale but not in bulk, i.e. metastable phases formed in a nucleation selective regime.
...
We will note that this not concept does notinclude phases obtained through topochemical routes, which may be metastable at all conditions, but are kinetically stabilized through a continuous transformation which allows them to stay in a local minimum. @parija2018TraversingEnergy
...
== Accuracy of computational methods in predicting theromdynamic stability
<s_sota_dft>

Accurate computational prediction of is key in correctly predicting the stability of a phase.
#textcite(<bartel2022ReviewComputational>) reviews computational approaches to computational theromdynamic predictions from density functional theory (DFT).
...
Formation energies from elements using the generalized gradient approximation (GGA) are quite high due (136 meV/atom) to the vaslty different chemical evironments between elemental reference states and the target phase.
This can be reduced by considering correcting these values using experiemental formation energies in a large dataset (1670 commpounds) of phases with different chemical environments, which can reduce this error to 81 meV/atom.

In cases where reactions energies only involve elements in similar chemical environments, the error can be futher reduced.
...
The reported 5th and 95th percentile signed errors $Delta E^"ox"_"GGA" - E^"ox"_"experimental"$ are -30 and 71 meV/atom, respectively, meaning that GGA is more likely to overestimate the stability of a ternary oxide with respect to binary counterparts underestimate it.

=== Gibbs free energy at higher temperatures

When considering the evolution from 0 K to higher temperatures, two main contribution to the Gibbs free energy need to be considered : vibrational entropy and, in systems exhibitng disorder, configurational entropy.

...
In the following, we attempt to identify through in-situ measurements reaction pathways in which metastable polymorphs form as intermidates through nucleation control.
...
Inspired by soft chemistry, which is sucessful at creating metastable phases through nucleation selectivity, we choose reactions where at least one precursor has a low melting point.
Considering Tamamn's rule, we can expect these reactions to occur at lower temperatures.
...
Using these critera, we screen for phases in the materials project with reported polymorphs including the following elements : 

- Mo (MoO3 has a melting point of 801degC)  @CRC2016
- Te (TeO2 has a melting point of 730degC)  @CRC2016
- V (V2O5 has a melting point of 730degC) @CRC2016
...
Solid state reactions of the candidates in @s_candidates was attempted in #lit[in-situ] XRD experiments, with the goal of identifying metastable polymporhps during the reaction process.
...
In the case of NiMoO4, beta-NiMoO4 (isotopic with alpha-MnMoO4) @sleightTransitionMetal forms directly.
...
For the case of FeVO4, shown in @f_META3_17_FeVO4, no intermediates are observed either, but the signal to noise ratio is lower than for the other reactions, making it difficult to identify small amounts of intermediates, meaning that a potential metastabel phase may have been missed.
...
The target phase LaVO4 forms otherwise without intermidates, but the more complicated nature of the reaction due to the presence of the dehydration step may hinder the detection of low concentration intermediates. 
...
This descrepancy causes the Rietveld refinement to be underfit the peak and therefore underestimates the phase fraction and results in a slight increase in the residual $R_(w p)$.
...
We note that, assuming this is a CuMoO4 polymorph and not another off-equilibrium compound, for which no other stoichiometries are known, that the simultateous onset of both polymorphs means that, by construction, at least one must be metastable at synthesis conditions.
...
CrVO4 #lit[-I] was first synthesized by #textcite(<touboul1995SynthesisChimie>) using a soft chemistry route, with a CrVO4#lit[-I] to CrVO4 #lit[-I]I transition observed at 660degC, which is consistent with the complete dissapearence of CrVO4 #lit[-I] we observe at 650degC.
// correct II to III
...
The CrVO4 #lit[-I] polymorph reaches a phase fraction larger than 5%.
...
Trirutile Ta2CuO6 is typcially synthesized from a freeze-dried #lit[Cu-Ta-oxalate] precursor at 700degC. @golubev2017StructuralMagnetic
...
All of these phases are otherwise reported in literature to be synthsized through soft chemistry routes where nucleation plays an important role, suggesting that nucleation selectivity had an impact in their synthesis. 
Furthermore, all of the metastable phases nucleate simultaneously with the stable polymorph, meaning that by construction at least one of the two must be metastable at synthesis conditions.
...
In this chapter, we attempt to bypass this by considering reaction with a low melting point precursor, which, following Tamman's rule, should result in a lower reaction temperature.
However, considering that the polymorphic transformation temperature, which is also broadly related to atomic mobility, may also be scaled, the benifits to this approach may be low.
...
Insipired by Tamman's rule, we can use the melting point of the target phase as a heuristic for the polymorphic transformation temperature, and using the approach from #textcite(<tan2026OriginReaction>)
...
- Case (a) is the worst case senario. Both based on tamman's rule and the metastable eutectic approach, the reaction onset is very close to the target melting point. 
- Case (b) is the case we considered here, where one low melting point precursor is used.
  In the case where Tamman's rule applies to the lower melting point precursor, this is a good approach.
  ...
  However, this approach may fail if using an average of the melting points of the precursors is a better predictor of the reaction onset using Tamman's rule, which is what we seem to observe in @f_onset_plot, or, depending on the specifics of the phase diagram or the stoichiometry of the target phase, if the metasteble eutectic approach by #textcite(<tan2026OriginReaction>) is a better predictor of the reaction onset.
...
- New systems can be considered, for which the phase diagram follows the ideal behaviour described above.
...
- Buliding up on the knowledge of #textcite(<zeng2024SelectiveFormation>), solid state metathesis reactions for which reaction energies are higher can be considered. This could result in a higher nucleation rate difference and thus a more selective formation of the metastable polymorph.
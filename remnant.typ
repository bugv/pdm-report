...
In this chapter, we consider the stability of ternary oxides phases predicted to be above the hull at 0 K by DFT, but which are reported to be synthesized in solid state reactions in the literature.
Barring any nanoscale stability or topochemical pathway, from the remnant metastability point of view, we would expect all of these phases to be stable at synthesis conditions.
In addition to testing this claim, we consider wether the predicted instability is due to 1. the inaccuracy of DFT, or 2. the fact that the phase is indeed metastable at 0 K, but is entropically stabilized at synthesis conditions.
...
We consider the text-mineddataset of solid state reactions yielding ternary oxides from #textcite(<kononova2019TextminedDataset>), which contains 3275 unique products. 
Note that only composition is provided, while specifics on polymorphism are not. 
Of these products, 1353 are ternary oxides, which we will focus on. 
Furthermore, in order to avoid redox chemistry which may decrease the accuracy of DFT predictions and may make synthesis difficult, we exclude all reactions involving gaseous reactants, leaving 804 condidate phases.

Energy above the hull at 0 K of these 804 is taken from the lowest lying experimental phase obtained for the given compositions from  Materials Project database @jain2013CommentaryMaterials, which reports DFT-computed energies at 0 K using a mixed GGA and R2SCAN functional scheme. @kingsbury2022FlexibleScalable
...
To separate phases that are stabel at 0 K, for which the reported instability is likely due to DFT inaccuracy, from phases which are indeed metastable at 0 K, we consider experimental thermochemical data, following the workflow described in @s_methods[Methods]. 
However, the difference in nature from DFT reported values and experimental data must be considered. 
The impact of atmospheric pressure on the stability of these phases is considered negligible, so internal energy $E$, given by DFT, and enthalpy $H$ are considered equivalent. 
The main difference now remains that experimental data is often reported at 298 K instead of 0 K.
@f_deviation shows that, for the three ternary oxide entries of the SGTE dataset@SGTE_LB which report enthalpy at 0 K, the difference between the enthalpy at 298 K and the enthalpy at 0 K is small and remains under 10 meV/at.
Furthermore, considering this time temperatures higher than 300 K, data from #textcite(<barin2008ThermochemicalData>) shown in @f_barin_deviation shows that deviations remain under 20 meV/at in the vast majority of cases up to synthesis conditions.
...
The experimental stability of these phases is assessed based on their reaction enthalpies and other thermodynamic properties.

@t_ox reports the reaction enthalpies $Delta H_298^"ox"$, entropies $Delta S_298^"ox"$, and Gibbs free energies $Delta G_298^"ox"$ of the reactions yielding these phases from their elementary oxides, as computationally obtained reaction energies.
...
#figure(
  ...
  caption:[Experimental and computational reaction energies of the reactions yielding the phases in @t_ox.
...
In the following sections, we first consider the phases which are stable at 0 K, and then consider the phases which are unstable at 0 K, and discuss their stability at synthesis conditions, and finally discuss phases for which experimental thermochemical data is not available in literature.
...
Experimental thermochemical data was not found for EuTi2O7.
For the other phases, reaction energies involving these competing phases is shown in @t_competing_reactions.
...
#figure(
  ...
  caption:[Reaction energies of the reactions yielding the phases in @t_ox with respect to competing phases, using experimental data from literature. Values reported with \* are extrapolated outside the range specified in the associated referenece.],
)
...
However, a thermodynamic assesment of the #lit[Mn-Ni-O] system from #textcite(<kjellqvist2010ThermodynamicAssessment>) does indicate stability of Mn2NiO4 in the 1000degC to 1200degC range.
...
Synthesis of Nd2NiO4 was unsucessful, which is consistent with experimental synthesis temperatures of 1350degC and 1400 degC reported in #textcite(<castro1995PhaseTransitions>) and #textcite(<murata2015FabricationCharacterization>), which is above the maximum temperature of 1200degC of the in-situ XRD apparatus.
...
We interestingly note that from Ta2CuO6 nucleates as two different polymorphs, which we will consider further in @s_Ta2CuO6.

#figure(
  image("fig/META2_4_1CuO_1WO3_quad_analysis_finalized.png", width: 75%),
  caption:[Reaction pathway from in-situ XRD synthesis of CuWO4 from MoO3 and NiO.],
)
<f_CuWO4_in_situ>

#figure(
  image("fig/META2_6_1CuO_1Ta2O5_quad_analysis_finalized.png", width: 75%),
  caption:[Reaction pathway from in-situ XRD synthesis of Ta2CuO6 from Ta2O5 and CuO.],
)
<f_Ta2CuO6_in_situ>

Calorimetry of CuWO4, Nd2NiO4, and Ta2CuO6 was also attempted, however due to strong drift in the measurement system the effect of the reaction on heat flow could not be resolved. 
